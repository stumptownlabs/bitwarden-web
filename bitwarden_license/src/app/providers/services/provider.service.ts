...
export class ProviderService {
    ...
    async addOrganizationToProvider(
        ...
        const encryptedOrgKey = await this.cryptoService.encrypt(
            orgKey.key,
            providerKey
        );
        ...
        const response = await this.apiService.postProviderAddOrganization(
            providerId,
            request
        );
    ...
    async detachOrganizastion(
        providerId: string,
        organizationId: string
    ): Promise<any> {
        await this.apiService.deleteProviderOrganization(
            providerId,
            organizationId
        );
        await this.syncService.fullSync(true);
