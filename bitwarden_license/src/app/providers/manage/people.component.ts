    ...
    implements OnInit
{
            ...
            this.route.queryParams.pipe(first()).subscribe(async (qParams) => {
                ...
                if (qParams.viewEvents != null) {
                    const user = this.users.filter(
                        (u) => u.id === qParams.viewEvents
                    );
                    if (
                        user.length > 0 &&
                        user[0].status === ProviderUserStatusType.Confirmed
                    ) {
                        this.events(user[0]);
                    }
                }
    ...
    async confirmUser(
        ...
        const providerKey = await this.cryptoService.getProviderKey(
            this.providerId
        );
        ...
        await this.apiService.postProviderUserConfirm(
            this.providerId,
            user.id,
            request
        );
    ...
    async bulkReinvite() {
        ...
        const filteredUsers = users.filter(
            (u) => u.status === ProviderUserStatusType.Invited
        );
        ...
        try {
            ...
            const response = this.apiService.postManyProviderUserReinvite(
                this.providerId,
                request
            );
            this.showBulkStatus(
    ...
    private async showBulkStatus(
        ...
        try {
            ...
            if (modal) {
                ...
                const keyedFilteredUsers: any = filteredUsers.reduce(
                    (a, x) => ({ ...a, [x.id]: x }),
                    {}
                );
